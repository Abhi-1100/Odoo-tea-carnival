...
const statusStyles: Record<string, string> = {
  draft: "bg-slate-500/20 text-brand-text border-slate-500/30",
  sent_to_kitchen: "bg-amber-100 text-amber-800 border border-amber-200 border-amber-500/30",
...
export default function BackendOrdersPage() {
  ...
  return (
    <div className="p-8 space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-brand-text">Orders</h1>
        ...
        <div className="xl:col-span-3 space-y-4">
          <div className="card p-4 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-2 relative">
                ...
                {actionOpen && (
                  <div className="absolute right-0 top-12 z-20 min-w-40 rounded-xl border border-brand-border bg-[#1b2030] shadow-2xl overflow-hidden">
                    <button onClick={archiveSelected} className="flex w-full items-center gap-2 px-4 py-3 text-sm text-brand-muted hover:text-brand-text hover:bg-brand-bg">
                      <Archive size={14} /> Archived
                    </button>
                    <button onClick={deleteSelected} className="flex w-full items-center gap-2 px-4 py-3 text-sm text-red-500 hover:text-brand-text hover:bg-brand-bg">
            ...
            <div className="flex items-center gap-2 text-xs text-brand-muted">
              <button onClick={toggleSelectAll} className="inline-flex items-center gap-2 rounded-md border border-brand-border px-3 py-1.5 hover:text-brand-text">
          ...
          <div className="card overflow-hidden">
            <div className="px-5 py-3 border-b border-brand-border bg-brand-bg/30 flex items-center justify-between">
              <h2 className="text-brand-text font-semibold">Orders</h2>
              <button className="text-brand-muted hover:text-brand-text text-xs inline-flex items-center gap-1" onClick={() => fetchOrders()}>
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3">
                        <button onClick={(e) => { e.stopPropagation(); toggleSelect(order.id); }} className="inline-flex h-5 w-5 items-center justify-center rounded border border-brand-border text-brand-muted hover:text-brand-text">
                      ...
                      <td className="px-4 py-3 text-brand-text text-sm font-semibold">₹{Math.round(totals.total).toLocaleString()}</td>
          ...
          {selectedOrder ? (
            <div className="card overflow-hidden sticky top-8">
              <div className="flex items-center justify-between px-5 py-4 border-b border-brand-border">
                <div>
                  <div className="text-xl font-bold text-brand-text">{selectedOrder.orderNumber}</div>
                  <div className="text-brand-muted text-sm">{new Date(selectedOrder.createdAt).toLocaleString("en-IN")}</div>
                </div>
                <div className="flex gap-2">
                  {selectedOrder.status === "draft" ? (
                    <>
                      <button onClick={() => updateStatus(selectedOrder.id, "cancelled")} className="px-3 py-1.5 rounded-md border border-red-500/30 text-red-500 text-sm">Draft</button>
              ...
              <div className="px-5 py-4 space-y-2 text-sm text-brand-muted border-b border-brand-border">
                <div className="flex justify-between"><span>Order number</span><span className="text-brand-text">{selectedOrder.orderNumber}</span></div>
                <div className="flex justify-between"><span>Date</span><span className="text-brand-text">{new Date(selectedOrder.createdAt).toLocaleDateString("en-IN")}</span></div>
                <div className="flex justify-between"><span>Session</span><span className="text-brand-text">{selectedOrder.session?.terminalName || "-"}</span></div>
                <div className="flex justify-between"><span>Customer</span><span className="text-brand-text">{selectedOrder.createdByUser?.name || "-"}</span></div>
              </div>

              <div className="px-5 pt-4 flex gap-2 border-b border-brand-border">
                <button className="px-3 py-2 text-sm bg-brand-bg rounded-t-md text-brand-text border border-brand-border border-b-0">Product</button>
                    ...
                    {selectedOrder.items.map((item) => {
                      ...
                      return (
                        <tr key={item.id} className="border-b border-brand-border/30">
                          <td className="py-2 text-brand-primary">{item.product.name} →</td>
                          <td className="py-2 text-brand-text">{item.quantity}</td>
                          <td className="py-2 text-brand-text">${item.unitPrice ? `$${item.unitPrice}` : "-"}</td>
                          <td className="py-2 text-brand-text">5%</td>
                          <td className="py-2 text-brand-text">Unit</td>
                          <td className="py-2 text-brand-text">₹{Math.round(lineTotal).toLocaleString()}</td>
                          <td className="py-2 text-brand-text">₹{Math.round(lineTotal + tax).toLocaleString()}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        <div className="flex justify-between text-brand-text font-bold text-lg"><span>Final Total</span><span>{Math.round(totals.total).toLocaleString()}</span></div>
                      </>
                    );
                  })()}
                </div>
