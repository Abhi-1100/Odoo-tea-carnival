...
import { Coffee, RefreshCw, Settings, X, LayoutGrid, Monitor, ClipboardList } from "lucide-react";
...
export default function POSLayout({ children }: { children: React.ReactNode }) {
  ...
  const isOrders = pathname === "/pos/orders";
      ...
      <header className="flex items-center justify-between px-6 py-3 bg-brand-card border-b border-brand-border shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-1 ml-4">
            ...
            <Link href="/pos" className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${isRegister ? "bg-brand-primary text-white" : "text-brand-muted hover:text-white hover:bg-brand-bg"}`}>
              <Monitor size={14} /> Register
            </Link>
            <Link href="/pos/orders" className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${isOrders ? "bg-brand-primary text-white" : "text-brand-muted hover:text-white hover:bg-brand-bg"}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/backend"><Button size="sm" variant="outline" icon={<Settings size={14} />}>Backend</Button></Link>
