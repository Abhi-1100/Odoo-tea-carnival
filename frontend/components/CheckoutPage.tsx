...
import { ChevronLeft, CreditCard, Banknote, QrCode, CheckCircle2, Copy, ArrowRight, Printer, Home } from 'lucide-react';
...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    const savedCart = localStorage.getItem('coffee-leo-cart');
    if (savedCart) {
  ...
  const handlePlaceOrder = () => {
    ...
    setTimeout(() => {
      const newOrderId = `CLEO-${Math.floor(1000 + Math.random() * 9000)}`;
      const letters = ['A', 'B', 'C', 'D'];
      const randomTable = `${letters[Math.floor(Math.random() * letters.length)]}${Math.floor(1 + Math.random() * 9)}`;
      const randomToken = Math.floor(10 + Math.random() * 89).toString();
      
      setOrderId(newOrderId);
      setTableName(randomTable);
      setQueueToken(randomToken);
      setIsProcessing(false);
      setIsSuccess(true);
      localStorage.removeItem('coffee-leo-cart');
      toast.success('Brewing your order!', {
        duration: 5000,
        ...
        <div className="relative z-10 max-w-lg w-full text-center">
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="bg-[#261e1b] border border-[#d4af37]/10 rounded-[32px] p-8 transform hover:scale-105 transition-transform duration-500">
              <span className="text-[10px] uppercase tracking-[4px] opacity-40 mb-2 block font-bold">Table Ref</span>
              <h2 className="text-5xl font-mono text-[#d4af37] font-bold">{tableName}</h2>
            </div>
            <div className="bg-[#261e1b] border border-[#d4af37]/10 rounded-[32px] p-8 transform hover:scale-105 transition-transform duration-500">
              <span className="text-[10px] uppercase tracking-[4px] opacity-40 mb-2 block font-bold">Queue Token</span>
              <h2 className="text-5xl font-mono text-[#f0dfdb] font-bold">{queueToken}</h2>
            </div>
          </div>

          <div className="bg-[#161614] border border-[#d4af37]/5 rounded-[32px] p-6 mb-12 flex items-center justify-between group">
            <div className="text-left">
              <span className="text-[9px] uppercase tracking-[4px] opacity-20 block mb-1 font-bold">Official Reference</span>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <button className="w-full sm:w-auto border border-[#d4af37]/30 text-[#d4af37] px-10 py-4 rounded-2xl font-bold uppercase tracking-widest hover:bg-[#d4af37]/5 transition-all flex items-center justify-center gap-3">
              <Printer size={18} />
              Print Token
            </button>
          </div>
